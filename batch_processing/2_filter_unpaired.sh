...
bowtie2 --local --very-sensitive-local -t -p 8 -x ${BOWTIE_DB} -U SE_PIPE | tee >(samtools view -hSb - | samtools flagstat - > log/bowtie_unpaired.flagstat ) | samtools view -hSb -f4 -F256 - | samtools fastq - | gzip > ${OUT_FILE}
